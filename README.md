...
the project is to provide a list of plugins/extensions to the popular opensource static code analyzer, SonarQube. The purpose of these plugins is to highlight code structures that may have a negative ecological impact: energy and resources over-consumption, "fatware", shortening terminals' lifespan, etc.

_creedengo_ is based on an evolving catalog of [good practices](docs/rules), for various technologies. As SonarQube plugins, _creedengo_ implements these best practices as rules for scanning your projects.

**Warning**: This project is still in its early evolving phase. Any feedback or contribution will be highly appreciated. Please
...
Code is parsed to be transformed as AST(Abstract Syntax Tree). AST will allow you to access one or more nodes of your code.
...
Each of these folders contains a file with the metadata of the rule, and its description by language.
...
To connect with us, kindly create an issue,first.
Then, if there is no response, contact ...
